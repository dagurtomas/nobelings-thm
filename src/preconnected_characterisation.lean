...
begin
  ...
  split,
  { intros hx,
    have hx' : (set.indicator U (cst_top X)) x = ⊤ := hx,
    have hxne0 : (set.indicator U (cst_top X)) x ≠ 0,
    { intros hux, rw hux at hx',
      have hto : ⊤ ≠ (0 : bool) := by tauto,
      apply hto,
      exact hx'.symm, },
    tidy },
  { intros hx,
    have h : (set.indicator U (cst_top X)) x = cst_top X x := by tidy,
    tidy },
end
...
def continuous_on_to_discrete_implies_constant {X Y : Type*} (α : set X) (f : X → Y)
  [topological_space X] : Prop := 
  continuous_on_to_discrete α f → ∀ a b : X, a ∈ α → b ∈ α → f a = f b 

lemma preconnected_space_iff_continuous_to_bool_implies_constant (X : Type*) [topological_space X] :
  preconnected_space X ↔ ∀ f : X → bool, continuous_to_discrete_implies_constant set.univ f :=
begin
  split,
  { intros hp,
    intros f hf a b ha hb,
    by_contra,
    let U := f ⁻¹' {f a},
    let V := f ⁻¹' ({f a}ᶜ),
...
lemma is_preconnected_iff_continuous_to_bool_implies_constant {X : Type*} (α : set X) 
  ...
  is_preconnected α ↔ ∀ f : X → bool, continuous_on_to_discrete_implies_constant α f :=
begin
  rw is_preconnected_iff_preconnected_space,
  rw preconnected_space_iff_continuous_to_bool_implies_constant α,
  split,
  { intros h f,
...
lemma is_preconnected_iff_continuous_to_discrete_implies_constant {X : Type*} (α : set X) 
  [topological_space X] [decidable_pred (∈ α)] : 
  is_preconnected α ↔ ∀ Y : Type, ∀ f : X → Y, continuous_on_to_discrete_implies_constant α f :=
begin
  rw is_preconnected_iff_continuous_to_bool_implies_constant α,
  ...
  { rw ← hib, 
    norm_num,
    simp at *,
    have hfb : {(set.indicator {f a} (cst_top' Y)) (f b)} = (set.indicator {f a} (cst_top' Y)) '' {f b} := by tidy,
